...
- **Structured Metadata**: 고유 값(`request_id`, `trace_id`)을 라벨도 본문도 아닌 **별도 필드**로 저장. cardinality 압박 없이 필터링.
- **Bloom filter (experimental)**: 본문 텍스트 존재 여부를 chunk 단위 bloom으로 사전 필터 → 고선택성 grep 쿼리 10배 빠름.
- **TSDB index 전환**: boltdb-shipper는 레거시. 신규는 `tsdb` schema 권장.

## 14. 연계 문서

- 상위 관측 전략 / Prometheus 연계: [./observability-deep-dive.md](./observability-deep-dive.md)
- 감사 로그(audit.k8s.io/v1) 수집 대상: [./security-deep-dive.md](./security-deep-dive.md) §11.3
- eBPF 기반 syscall/네트워크 관측 병행: [../kernel/ebpf-deep-dive.md](../kernel/ebpf-deep-dive.md)
- Ceph RGW 스토리지 백엔드(ML-21 용량 사고 배경): [../hw/ceph-storage-deep-dive.md](../hw/ceph-storage-deep-dive.md)
- Airflow/Kubeflow 로그 소스: [./mlops-stack-deep-dive.md](./mlops-stack-deep-dive.md), [./inference-serving-deep-dive.md](./inference-serving-deep-dive.md)
